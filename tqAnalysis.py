...
"""
calculate torque T(r) for specified azimuthal modes.
parameters (dens, r_sup, r_inf, r_med, theta) have shape (nr, ns)
`modes` has shape (n_modes)
returns array of shape (len(modes), nr)
"""
def computeTorque(mb, secr, sect, dens, r_sup, r_inf, r_med, theta, modes, indirect_term):
    nr, ns = r_med.shape
    n_modes = len(modes)
    psi = theta - sect
    ...
    # torque per cell, shape (nr, ns)
    cell_tq = area * scale * force
...
def main():
    ...
    while True:
        ...
        tqFourier.append(computeTorque(mb, secr[i], sectheta[i], dens, r_sup, r_inf, r_med, theta, np.arange(11), True))
        totalTqDirect.append(np.sum(computeTorque(mb, secr[i], sectheta[i], dens, r_sup, r_inf, r_med, theta, [0], False)))
    ...
    print 'saving'
    np.save('parsedDiagnostics/tqFourier', tqFourier)
